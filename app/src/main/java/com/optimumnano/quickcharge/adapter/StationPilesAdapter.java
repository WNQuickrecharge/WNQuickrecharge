    ...
    protected void convert(BaseViewHolder helper, final PileBean item) {
        ...
        switch (status) {
            case PROMPTLY_CHARGE:
                ...
                final String pileNo1 = finalFirstFreeGun.getPileNo();
                pileStatus.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        OrderManager.getGunInfo(pileNo1+finalFristFreeGunNo, new ManagerCallback<RechargeGunBean>() {
                            @Override
                            public void onSuccess(RechargeGunBean returnContent) {
                                ...
                                bundle.putString("gunNo",pileNo1+finalFristFreeGunNo);
