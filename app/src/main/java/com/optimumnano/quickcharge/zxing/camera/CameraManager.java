...
package com.optimumnano.quickcharge.zxing.camera;
...
public final class CameraManager {
	...
	public Rect getFramingRectPosition() {
		Point screenResolution = configManager.getScreenResolution();
		if (framingRect == null) {
			if (camera == null) {
				return null;
			...
			if (width < MIN_FRAME_WIDTH) {
				...
				// width = MAX_FRAME_WIDTH + 200;
				if (!(width <= MAX_FRAME_WIDTH + 200)) {
					width = MAX_FRAME_WIDTH + 200;
				}
			}
			...
			if (height < MIN_FRAME_HEIGHT) {
				...
				// height = MAX_FRAME_HEIGHT + 250;
				if (!(height <= (MAX_FRAME_HEIGHT + 250))) {
					height = MAX_FRAME_HEIGHT + 250;
				}
			...
			int leftOffset = (screenResolution.x - width) / 2;
			int topOffset = (screenResolution.y - height) / 16*5 - (int) context.getResources().getDisplayMetrics().density * 50;
//			int topOffset= (screenResolution.y - height) /16*5;
			framingRect = new Rect(leftOffset, topOffset, leftOffset + width, topOffset + height);
			Log.d(TAG, "Calculated framing rect: " + framingRect);
		}
		return framingRect;
